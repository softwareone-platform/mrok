...
# from mrok.datastructures import Event, HTTPResponse
...
#     response: reactive[HTTPResponse | None] = reactive(None, recompose=True)

#     def compose(self):
#         with Vertical():
#             if self.response:
#                 # yield Static("The request is selected", classes="summary")
...
#         summary_text = Text()
#         summary_text.append(f"{self.response.request.method} ", style="bold cyan")
#         summary_text.append(f"{self.response.request.url}\n", style="bold")
#         summary_text.append("Status: ", style="bold")
#         summary_text.append(
#             f"{self.response.status} ", style=f"bold {color_for_status(self.response.status)}"
#         )
#         summary_text.append(f"• Duration: {self.response.duration * 1000} ms", style="dim")
#         req_headers_formatted = "\n".join(
#             f"[yellow]{k}[/]: {v}" for k, v in self.response.request.headers.items()
#         )
#         res_headers_formatted = "\n".join(
#             f"[yellow]{k}[/]: {v}" for k, v in self.response.headers.items()
#         )
...
#                     body = self.response.body.decode()
#                     if "/" in ct:
#                         if ";" in ct:
#                             ct, _ = ct.split(";", 1)
#                         _, lexer = ct.split("/", 1)
...
#                     body = self.response.request.body.decode()
#                     if "/" in ct:
#                         if ";" in ct:
#                             ct, _ = ct.split(";", 1)
#                         _, lexer = ct.split("/", 1)
...
#                         body = json.dumps(json.loads(body), indent=4)
#                     yield VerticalScroll(
#                         Static(Syntax(body, lexer, theme="monokai", word_wrap=True))
#                     )
#                 else:
#                     yield Static("No body")
#             with TabPane("Request parameters", id="params"):
#                 yield VerticalScroll(Static("No parameters"))
...
#     def __init__(self, subscriber_port: int):
#         super().__init__()
#         self.refresh_interval = 10.0
#         self.requests: dict[str, HTTPResponse] = {}
...
#                 yield RequestDetailCard(id="req-card")
#         yield Footer()

#     async def on_mount(self):
#         table = self.query_one("#requests", DataTable)
#         table.add_columns("Method", "Status", "Path", "Duration (ms)")
...
#                 log(e)
#         log("message worker exited")

#     def on_data_table_row_selected(self, event: DataTable.RowSelected) -> None:
#         req_id = event.row_key.value
#         if req_id:
#             card = self.query_one("#req-card", RequestDetailCard)
#             card.response = self.requests.get(req_id)
