...
def run_sidecar(service_name: str, identity_file: str, target_addr: str | Path | tuple[str, int]):
    config = MrokBackendConfig(ForwardApp(target_addr), service_name, identity_file)
...
def run(
    ...
    start_fn = partial(run_sidecar, service_name, identity_file, target_addr)
