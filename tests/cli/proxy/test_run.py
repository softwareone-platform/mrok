...
def test_run(mocker: MockerFixture):
    mocker.patch(
        "mrok.cli.commands.proxy.run.get_logging_config",
        return_value={"logging": "config"},
    )
    mocked_app = mocker.MagicMock()
    mocked_standalone_app = mocker.patch(
        "mrok.cli.commands.proxy.run.StandaloneApplication", return_value=mocked_app
    )
    mocked_proxy_app = mocker.MagicMock()
    mocked_proxy_ctor = mocker.patch(
        "mrok.cli.commands.proxy.run.ProxyApp",
        return_value=mocked_proxy_app,
    )
    runner = CliRunner()
    ...
    mocked_proxy_ctor.assert_called_once_with("my-identity.json")
