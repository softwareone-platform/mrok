...
async def test_get(
    ...
    httpx_mock.add_response(
        method="GET",
        url=f"{settings.ziti.base_urls.management}/edge/management/v1/services/service123",
        json={"data": {"id": "service123", "name": "svc"}},
    )
    httpx_mock.add_response(
        method="GET",
        url=f"{settings.ziti.base_urls.management}/edge/management/v1/services/service123/policies",
...
async def test_fetch_ca_certs(
    ...
    httpx_mock.add_response(
        method="GET",
        url=f"{settings.ziti.base_urls.management}/edge/management/v1/.well-known/est/cacerts",
...
async def test_password_auth(
    ...
    httpx_mock.add_response(
        method="POST",
        url=f"{settings.ziti.base_urls.management}/edge/management/v1/authenticate?method=password",
        match_json={
...
async def test_identity_auth(
    ...
    settings = settings_factory(
        ziti={
            "base_urls": {"management": "https://ziti.example.com"},
    ...
    httpx_mock.add_response(
        method="POST",
        url=f"{settings.ziti.base_urls.management}/edge/management/v1/authenticate?method=cert",
        status_code=200,
        json={"data": {"token": "auth-token"}},
    )
    httpx_mock.add_response(
...
async def test_enroll_identity(
    ...
    httpx_mock.add_response(
        method="POST",
        url=f"{settings.ziti.base_urls.client}/edge/client/v1/enroll?method=ott&token=jti",
        status_code=200,
        match_content=b"csr",
